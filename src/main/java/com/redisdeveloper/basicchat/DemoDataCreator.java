...
//	// private static final List<String> DEMO_USERNAME_LIST = Arrays.asList("Pablo",
//	// "Joe", "Mary", "Alex");
//	private static final List<String> DEMO_USERNAME_LIST = Arrays.asList("Joe", "Mary", "Alex", "Pablo");
...
//			// Some rooms have pre-defined names. When the clients attempts to fetch a room,
...
//			for (User user : users) {
//				List<User> otherUsers = users.stream().filter(x -> x.getId() != user.getId())
...
//					}
//					addMessage(privateRoomId, String.valueOf(otherUser.getId()), getRandomGreeting(),
//							generateMessageDate());
//				}
//			}
//
...
//
//		// Yeah, bcrypt generally ins't used in .NET, this one is mainly added to be
//		// compatible with Node and Python demo servers.
//		String hashedPassword = encoder.encode(DEMO_PASSWORD);
//
//		redisTemplate.opsForValue().increment("total_users").intValue();
...
//
//		String roomsKey = String.format("user:%s:rooms", userId);
//		redisTemplate.opsForSet().add(roomsKey, "0");
//
//		return new User(userId, username, false);
//	}
//
//	private String getPrivateRoomId(String userId1, String userId2) {
//
//		int compare = userId1.compareTo(userId2);
//		if (compare < 0) {
//			System.out.println(userId1 + " is before " + userId2);
//			return String.format("%s:%s", userId1, userId2);
//		} else {
//			return String.format("%s:%s", userId2, userId1);
//		}
//	};
//
//	private Room createPrivateRoom(String user1, String user2) {
...
//
//		return new Room(roomId, (String) redisTemplate.opsForHash().get(key1, "username"),
